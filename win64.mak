# vlc_installer/win64.mak

VER?=$(shell cat version.txt)
NSI?=$(wildcard *.nsi)
APP?=$(shell basename *.nsi .nsi)
PREFIX?=$(APP)-$(VER)-win64
EXE?=$(PREFIX).exe
NSHS?=$(wildcard ../nshlib/*.nsh)
APP_ZIP?=$(PREFIX).zip
SRC_ZIP?=$(PREFIX)-src.zip
APP_FILES=$(EXE) $(wildcard LICENSE.* CHANGELOG.* README.*)
SRC_FILES=$(APP_FILES) $(NSHS) $(wildcard Makefile *.mak *.nsi *.nsh version.txt)
...
$(EXE):	$(NSI) $(NSHS) version.txt
	makensis /V4 /O$(APP).log /DPRODUCT_VERSION=$(VER) /DPRODUCT_OUTFILE=$@ /DINSTALL_IN_PROGRAMFILES64=1 $<
	chmod a+x *.exe
